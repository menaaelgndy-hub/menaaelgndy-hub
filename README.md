# Hi there, I'm Menna El Gandy 👋
## Odoo Functional Consultant | Business & Data Analyst

### 🛠️ Expertise
- **Odoo ERP:** Functional Configuration, Inventory (Spare Parts) & POS
- **Data Analytics:** Power BI (Reporting), Data Mining & SQL Fundamentals
- **Programming & Web:** Python (Basics), HTML5, CSS3
- **Analysis:** Business Workflow Mapping & System Analysis (DFD, ERD)
- **IT Foundations:** Software Engineering & Database Management
- **Productivity Tools:** Advanced Microsoft Office (Excel, Word)
- **Languages:** Proficient in English (Written & Spoken)

### 📁 Projects & Professional Experience
- **BMW Dataset Analysis:** Applied Data Mining techniques on BMW datasets for academic insights.
- **Automotive Spare Parts System:** End-to-end Odoo implementation for warehouse and e-commerce.
- **Data Insights:** Building interactive dashboards and reports using Power BI.
- **Rosetta Flowers:** System analysis and Odoo functional configuration.
- **Digital Presence:** Established and managed professional social media presence for business branding.

### 🏆 Top Projects
- **[Comprehensive Odoo ERP System - Rosetta Flowers Project](https://github.com/menaaelgndy-hub/Rosetta-Flowers-Odoo-ERP/blob/main/Flower_shop.pdf)
  
### 📫 Connect with me:
[<img src="https://img.shields.io/badge/linkedin-%230077B5.svg?style=for-the-badge&logo=linkedin&logoColor=white" />](https://www.linkedin.com/in/menna-el-gandy)
